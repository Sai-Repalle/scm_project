        ...
        if user_dict.get(key, None): 
            if not os.path.exists(user_dict[key]):
                if key == "CONFIG_DEF_FILE":
                    Path(os.path.join(user_dict['CONFIG_DIR'], user_dict[key])).touch()
                else:
                    Path(os.path.join(user_dict['CONFIG_HASH_DIR'], user_dict[key])).touch()
            else:
...
def validate_unsupported_resources(user_resources) -> Set:
    unsupported_resources = ({*user_resources} - defaults.DEFAULT_PARAMTERS) - defaults.SUPP_RES
...
CONFIG_FILE_PATH = os.path.join(CONFIG_DIR_PATH, "scm\settings\settings.json")
